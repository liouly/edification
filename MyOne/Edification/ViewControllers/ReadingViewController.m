...
#import "HTTPTool.h"
#import "ReadingNewsModule.h"
#import "ReadNewsCell.h"
#import <UIImageView+WebCache.h>
#import "ReadNewsDetailViewController.h"

@interface ReadingViewController () <RightPullToRefreshViewDelegate, RightPullToRefreshViewDataSource,UITableViewDelegate,UITableViewDataSource>
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
	...
	if (self) {
		...
		// 底部导航item
		UITabBarItem *tabBarItem = [[UITabBarItem alloc] initWithTitle:@"头条" image:nil tag:0];
...
- (UITableView *)tableView
{
    if (nil == _tableView) {
        ...
        [_tableView registerClass:[ReadNewsCell class] forCellReuseIdentifier:@"ReadNewsCell"];
    }
    return _tableView;
}
...
{
    ReadNewsCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ReadNewsCell"];
    
    ReadingNewsModule *module = [self.dataArray objectAtIndex:indexPath.row];
    [cell.iconView sd_setImageWithURL:[NSURL URLWithString:module.thumbnail_pic_s]];
    cell.titleLabel.text = module.title;
    cell.authorLabel.text = module.author_name;
    
    cell.titleLabel.textColor = DawnTextColor;
    cell.titleLabel.nightTextColor = NightTextColor;
    cell.authorLabel.textColor = DawnTextColor;
    cell.authorLabel.nightTextColor = NightTextColor;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ReadNewsDetailViewController *detailViewCtrl = [[ReadNewsDetailViewController alloc] init];
    detailViewCtrl.webUrl = module.url;
...
- (void)requestReadingContentAtIndex:(NSInteger)index {
	...
	[HTTPTool requestReadingContentByDate:date lastUpdateDate:lastUpdateDate success:^(AFHTTPRequestOperation *operation, id responseObject) {
		if ([responseObject[@"error_code"] integerValue] == 0) {
            ...
            NSArray *array = [[responseObject objectForKey:@"result"] objectForKey:@"data"];
            for (NSDictionary *dic in array) {
                ReadingNewsModule *returnReadingEntity = [ReadingNewsModule objectWithKeyValues:dic];
                [self.dataArray addObject:returnReadingEntity];
            }
